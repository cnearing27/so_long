...
int	walls_around(t_map	*map)
{
	...
	i = 0;
	while (i < map->width) //проверка первой и полседней строки
	{
		if(map->data[0][i] == '1' &&
	...
	i = 1;
	while (i < map->height - 1) //проверка средних строк
	{
		if(map->data[i][0] == '1' &&
...
int	entities(t_map	*map)
{
	...
	i = 1;
	if (map->exits_count == 0 || map->players_count == 0 || map->collectibles_count == 0)
		return (0);
	return (1);
}
